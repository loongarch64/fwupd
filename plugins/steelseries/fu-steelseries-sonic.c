				     ...
				     GError **error)
{
	...
	if (g_getenv("FWUPD_STEELSERIES_SONIC_VERBOSE") != NULL)
		fu_common_dump_raw(G_LOG_DOMAIN, "WirelessStatus", data, sizeof(data));
	if (!fu_common_read_uint8_safe(data, sizeof(data), 0x0, &value, error))
		return FALSE;
	*status = value;
...
fu_steelseries_sonic_battery_state(FuDevice *device, guint16 *value, GError **error)
{
	...
	if (!fu_common_write_uint8_safe(data, sizeof(data), 0x1, bat_mode, error))
		return FALSE;
	...
	if (g_getenv("FWUPD_STEELSERIES_SONIC_VERBOSE") != NULL)
		fu_common_dump_raw(G_LOG_DOMAIN, "BatteryState", data, sizeof(data));
	if (!fu_common_read_uint16_safe(data, sizeof(data), 0x0, value, G_LITTLE_ENDIAN, error))
		return FALSE;
...
fu_steelseries_sonic_init(FuSteelseriesSonic *self)
{
	...
	fu_device_set_firmware_size(FU_DEVICE(self),
				    STEELSERIES_SONIC_FIRMWARE_SIZE[0] +
					STEELSERIES_SONIC_FIRMWARE_SIZE[1] +
					STEELSERIES_SONIC_FIRMWARE_SIZE[2]);
